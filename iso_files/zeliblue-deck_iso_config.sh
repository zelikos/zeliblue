...
IMAGE_INFO="$(cat /usr/share/zeliblue/image-info.json)"
IMAGE_TAG="$(jq -c -r '."image-tag"' <<<"$IMAGE_INFO")"
# IMAGE_REF="$(jq -c -r '."image-ref"' <<<"$IMAGE_INFO")"
# IMAGE_REF="${IMAGE_REF##*://}"
IMAGE_REF="ghcr.io/zelikos/zeliblue-deck"
...
favorite-apps = ['anaconda.desktop', 'documentation.desktop', 'discourse.desktop', 'org.gnome.Nautilus.desktop', 'org.gnome.Epiphany.desktop']
EOF
...
# Zeliblue
tee /etc/anaconda/profile.d/zeliblue.conf <<'EOF'
# Anaconda configuration file for Zeliblue
...
sed -i 's| Fedora| Zeliblue|' /usr/share/anaconda/gnome/fedora-welcome || true
